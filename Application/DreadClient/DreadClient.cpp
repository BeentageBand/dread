...
void DreadClient::uploadFromPersistence(void) {
   ...
   if (!modem->isConnected()) {
      hid.setLed(0);
   } else if (DreadClient::send_packet && uploadPacket() && persistence->deprecateFilename()) {
...
bool DreadClient::uploadPacket(void) {
    ...
    bool result = persistence->beginTransaction(READ_TRANSACTION);                     /*Stores the return value of gprs methods.*/

    Locale & locale = Locale::get();
    static uint8_t datetime[DATETIME_SIZE];

    locale.getDateTime(datetime);
    uint32_t file_length = persistence->getSize();               /*Variable that stores the backup file size. Up to 2^31*/

    //logger->printNumber(file_length,DEC);                  /*Prints file_length, it will decrease with each packet.*/
    //logger->println();

    list_length+=DATETIME_SIZE;                  /*add length of date + \r: 19 bytes*/

    result=modem->openPacketSession();                     /*Preset result as true. It will turn false if modem instructions fails
