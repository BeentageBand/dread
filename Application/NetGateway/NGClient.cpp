...
NGClient::NGClient(char const *client_name, Socket &socket,
                   NetManager &net_manager, Persistence &persistence,
                   Print &logger)
...
void NGClient::scanNet(union Request &req) {
  if (0 == net_manager->availableAddresses())
    return;
...
void NGClient::handleGetRecord(union Request &req, union Response &res) {
  if (!net_manager->contains(res.addr))
    return;

  delayMicroseconds(100);
  uint8_t result = persistence->beginTransaction(WRITE_TRANSACTION);
  if (result) {
    // Prints in file the following format: ID \t RFIDContent(as a decimal
    // number) \t date (dd/mm/yyyy) \t hour (hh:mm) \t Serialnum uint32_t record
    // = *reinterpret_cast<uint32_t *>(res.msg);
