...
inline void SoftwareSerial::tunedDelay(uint16_t delay) {
  ...
  asm volatile("sbiw    %0, 0x01 \n\t"
      ...
      : "+w" (delay), "+a" (tmp)
      : "0" (delay)
      );*//*: "+w" (delay), "+a" (tmp)  should change to : "+r" (delay), "+a" (tmp)*/
}
...
void SoftwareSerial::recv() {
  ...
  if (_inverse_logic ? rx_pin_read() : !rx_pin_read()) {
    ...
    if (((_receive_buffer_tail + 1) & _SS_BUFF_MASK) != _receive_buffer_head) {
      // save new data in buffer: tail points to where byte goes
      _receive_buffer[_receive_buffer_tail] = d; // save new byte
      _receive_buffer_tail = (_receive_buffer_tail + 1) & _SS_BUFF_MASK;
    } else {
...
ISR(PCINT1_vect) { SoftwareSerial::handle_interrupt(); }
#endif

#if defined(PCINT2_vect)
ISR(PCINT2_vect) { SoftwareSerial::handle_interrupt(); }
#endif

#if defined(PCINT3_vect)
ISR(PCINT3_vect) { SoftwareSerial::handle_interrupt(); }
#endif
...
int SoftwareSerial::read() {
  ...
  _receive_buffer_head = (_receive_buffer_head + 1) & _SS_BUFF_MASK;
  return d;
}
