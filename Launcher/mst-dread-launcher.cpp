...
/*
        ...
        After several field work, the Master and Slave software version have
   some bugs. When the network falls, some DReads in the network may crash.
...
static NGClient ngclient(client_name, socket, net_manager, persistence,
                         SoftSerial);
...
/* Description:
 ...
 slave_select. Halts SC16 device by set HIGH SC16 slave_select pin 10. Uses HID
 functions to determine if SD card is inserted, also prevents the program to
 ...
 * Notes:
        - Has a saveFileName function, description further but may not be
 implemented because is redundant and fats the program size up.
        - Setting SC16 SS may be unnecessary, check SC16IS740.cpp to make sure
 set SC_SS high it's necessary.
        - If the DRead doesn't have a Micro SD card properly inserted, the
