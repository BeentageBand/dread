...
static Scheduler::Subscription connection_subscription(DreadClient::CheckTime,
                                                       5 * INT_MINUTES);
...
//** Function: ConfigSD()
/* Description:
   INLINE function which configures SPI COMM (speed,slave_select) for micro SD
 card memory usage. Sets the half speed SPI SCK and uses pin 8 as SD
 slave_select. Halts RFID device by set HIGH Mifare slave_select pin 10. Uses
 HID functions to determine if SD card is inserted, also prevents the program to
 initialize without SD card. The program will start when the micro SD card is
 inserted properly. Variables required:
   - SD         object for SD communication.
   - SoftSerial      debugging proposals.
 * Notes:
   - Has a saveFileName function, description further but may not be implemented
 because is redundant and fats the program size up.
   - Setting RFID SS may be unnecessary, check MifareLite.cpp to make sure set
 MFR_SS high it's necessary.
   - If the DRead doesn't have a Micro SD card properly inserted, the program
...
inline void ConfigGPRS() {
  ...
  /*Blinks green led if was able to register to network. Blinks red led if
   * not.*/
  ...
  /*  Deprecated in V 1.3
     Reasons: The config file was executed to set the SIM900 modem parameter in
     order to use the GPRS with APN account. There was a problem with this
     structure, the configuration is Service Provider dependent and it was done
     once (during setup). Therefore, if Service Provider wasn't found, there was
     no retry on configuration parameter. Even if the GPRS was available, Sim900
     modem couldn't achieve a successful connection. DownloadGPRS has only to
     set parameter found in config.txt, so config.txt was mixed to ID.txt,
     helping the spot of
  */
  /*
     buffer[0]='\"';
     ...
     SoftSerial.println(gsm_retry,DEC);
     buffer[++counter]='\"';
     gprs.setAtParameter(FTP_GET_Filename,OK,buffer,counter+1,GSM_TOUT);//send
     the filename command with file_download parameter
   */
