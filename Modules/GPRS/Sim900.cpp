...
void Sim900::begin(const uint16_t baud_rate, const uint8_t rst_pin,
                   const uint8_t nopowerdown_pin) {
  uint8_t ptr;
...
boolean Sim900::matchResponse(const uint8_t *expected_response, uint8_t &ptr) {
  ...
  while (_hard_port->available() > 0 && !result) {
    ...
    // Compare, if the char is equal, the pointer advance, otherwise, it resets
    // to zero.
...
boolean Sim900::setAtParameter(const uint8_t *atcmd,
                               const uint8_t *expected_reponse,
                               const uint8_t *set_parameter,
                               const uint8_t parameter_size,
                               const uint32_t timeout) {
  _hard_port->flush();
  printP(AT);
  printP(atcmd);
...
boolean Sim900::waitFor(const uint8_t *expected_response,
                        const uint32_t timeout) {
...
boolean Sim900::waitForC(const uint8_t *expected_response,
                         const uint32_t timeout) {
  uint32_t time = millis();
  boolean success = false;
  uint8_t ptr = 0;
