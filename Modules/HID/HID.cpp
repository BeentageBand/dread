...
HID::HID(uint8_t const buzz_pin, uint8_t const red_led_pin, uint8_t const greenblue_led_pin)
  : buzz_pin(buzz_pin), red_led_pin(red_led_pin), greenblue_led_pin(greenblue_led_pin), led_register(ACQUIRE_MASK)
{
...
HID & HID::get(void)
{
  return HID::get(HID_BUZZ, HID_RLD, HID_BLD);
}

HID & HID::get(uint8_t const buzz_pin, uint8_t const red_led_pin, uint8_t const green_led_pin)
{
...
void HID::CheckTime(void)
{
   ...
   if(tick>=0)///////////////////CHANGE to 60 further. 20 is for debugging purposes!!!
   {
...
bool HID::isLocked(void)
{
  return static_cast<bool>(ReadBit(led_register, 7));
}

void HID::acquire(void)
{
  ClearBit(led_register, 7);
}

void HID::release(void)
{
  SetBit(led_register, 7);
}

void HID::onCheckTime(void)
{
  uint8_t red = ReadBit(led_register, RED_BIT) ^ ReadBit(led_register, TOGGLE_RED_BIT);
  uint8_t green_blue = ReadBit(led_register, GRN_BLU_BIT) ^ ReadBit(led_register, TOGGLE_GRN_BLU_BIT);
...
void HID::successfulChime(void)
{
  ...
  digitalWrite(buzz_pin, HIGH); //BEEEEEP, if commented is for debugging purposes, it's annoying!!!
