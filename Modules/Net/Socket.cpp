...
Socket::Socket(Print &port, Print &logger) : port(&port), logger(&logger) {}

void Socket::send(union Request &req) { port->write(req.raw, sizeof(req.raw)); }
...
uint8_t Socket::process(union Request &req, uint32_t timeout) {
  return listenMsg(req.raw, REQUEST_HEADER, MSG_FOOTER, sizeof(req.raw),
                   timeout);
}

uint8_t Socket::process(union Response &res, uint32_t timeout) {
  return listenMsg(res.raw, REQUEST_HEADER, MSG_FOOTER, sizeof(res.raw),
                   timeout);
}

uint8_t Socket::listenMsg(uint8_t *msg, uint8_t header, uint8_t footer,
  ...
  logger->write("Time:\n");
  logger->write(time);
  // Return if the message matches length,initial and final characters, in other
  // words, matches format.
