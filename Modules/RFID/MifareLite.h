...
#define PICC_REQIDL                                                            \
  0x26 // Search the cards that not into sleep mode in the antenna area
...
public:
  ...
  uint8_t Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey,
               uint8_t *buff, uint8_t *serNum);
  // uint8_t Auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey,
  // uint8_t *serNum);
