 ...
 */
//#warning you commented void SdFile::ls(uint8_t flags, uint8_t indent)
/* {
  dir_t* p;
 ...
 */
//#warning you commented void SdFile::printDirName(const dir_t& dir, uint8_t width) 
/*{
  uint8_t w = 0;
  for (uint8_t i = 0; i < 11; i++) {
    if (dir.name[i] == ' ')continue;
 ...
 */
//#warning you commented void SdFile::printFatDate(uint16_t fatDate)
/* {
 ...
 */
//#warning you commented print functions void SdFile::printFatTime(uint16_t fatTime)
/* {
...
//#warning you commented print functions void SdFile::printTwoDigits(uint8_t v)
/* {
  char str[3];
  str[0] = '0' + v/10;
  str[1] = '0' + v % 10;
 ...
 *
 */
void SdFile::write(const void* buf, uint16_t nbyte) {
  ...
  while (nToWrite > 0) {
    ...
    if (blockOfCluster == 0 && blockOffset == 0) {
      ...
      if (curCluster_ == 0) {
        ...
        uint32_t next;
        if (!vol_->fatGet(curCluster_, &next)) return;
        if (vol_->isEOC(next)) {
