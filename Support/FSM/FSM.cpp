...
Machine::Machine(uint8_t const initial_state, Chart *PROGMEM chart,
                 uint8_t const nstates)
    : initial_state(initial_state), current_state(NO_STATE), nstates(nstates),
      chart(chart) {}
...
bool Machine::transition(Transition &transition, State const &next_st) {
  if (NULL == transition.guard || transition.guard(*this, next_st))
    return false;
