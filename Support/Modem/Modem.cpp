...
static const uint8_t plusFTP_PUT[] PROGMEM =
    "FTPPUT:1,1,\r"; // next goes lengthLENGTH
...
void Modem::updatePacketName(char const *filename, uint8_t const fsize) {
  sim900->setAtParameter(FTP_PUT_Filename, OK, (uint8_t *)filename, fsize,
                         GSM_TOUT);
...
void Modem::sendPacketData(Print &data_print, uint16_t const data_size) {
  ...
  bool result =
      sim900->setAtParameter(FTP_PUT_Length, plusFTP_PUT_Length, data_size,
