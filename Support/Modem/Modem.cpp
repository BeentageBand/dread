...
Mode::Modem(File & file, Sim900 & sim900, Print & logger)
: file(&file), sim900(&sim900), logger(&logger) {}

bool Modem::isConnected(void) { return cgreg_flag; }
...
void Modem::updatePacketName(char const * filename, uint8_t const fsize) {
    sim900.setAtParameter(FTP_PUT_Filename,OK, filename,fsize,GSM_TOUT);   /*Sends the filename command with file_name copied on buffer*/
}
...
void Modem::sendPacketData(Print & data_print, uint16_t const data_size) {
    ...
    return result && sim900.waitFor(OK,GSM_TOUT) && sim900.waitFor(plusFTP_PUT,GSM_TOUT);
}
