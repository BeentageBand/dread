...
Scheduler::Subscription::Subscription(void (*subscription)(void),
                                      uint16_t const timeout)
    : subscription(subscription), timeout(timeout), next(NULL) {}

Scheduler::Scheduler(void) : subscriptions(NULL), tick(0) {}
...
void Scheduler::subscribe(Scheduler::Subscription *const subscription) {
  ...
  while (NULL != it->next) {
    if (it->subscription == subscription->subscription)
      return;
    it = it->next;
  }
